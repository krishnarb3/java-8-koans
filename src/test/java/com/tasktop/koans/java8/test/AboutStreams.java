...
public class AboutStreams {
	...
	public void java8_filterList() {
		...
		List<String> actualList = list.stream().filter(element -> element.equals("a")).collect(Collectors.toList());
	...
	public void java8_mapListToOtherType() {
		...
		Function<? super String, ? extends Integer> mapper = Integer::valueOf;
	...
	public void java8_findListElement() {
		...
		String result = list.stream().filter(element -> element.equals("a.1")).findFirst().get();
